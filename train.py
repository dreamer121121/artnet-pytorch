...
def train(params, train_loader, validation_loader):
    ...
    for epoch in range(start_epoch,params.getint('num_epochs')):
        ...
        log("train data num:{}".format(len(train_loader) * train_loader.batch_size),file=log_stream)
        ...
        with torch.no_grad():
            for batch_index, (frames, label) in validating_progress:
                ...
                prediction, label = prediction.to('cpu'), label.to('cpu')
                correct += prediction.eq(torch.LongTensor(label)).sum().item()
            log("val correct on epoch end = "+str(correct),file=log_stream)
            log("val data num:{}".format(len(validation_loader) * validation_loader.batch_size),file=log_stream)
            log("validation loss on epoch end = "+str(validating_loss),file=log_stream)
